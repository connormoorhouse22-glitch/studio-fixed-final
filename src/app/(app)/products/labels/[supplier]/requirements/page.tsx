...
// Helper to un-slug and title-case the supplier name from the URL
const formatSupplierName = (slug: string) => {
    return slug
        .replace(/-/g, ' ')
        .replace(/\b\w/g, char => char.toUpperCase());
}

export default function DynamicLabelRequirementsPage() {
  ...
  const supplierName = formatSupplierName(params.supplier as string);
    ...
    if (state.message) {
        if (state.success) {
            toast({
                ...
                description: state.message || `Your quote request has been sent to ${supplierName}.`,
            });
  ...
  }, [state, router, toast, supplierName]);
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <h2 className="text-3xl font-bold tracking-tight">{supplierName} Production</h2>
       ...
       <Card>
            ...
            <CardContent>
                <SubmitButton buttonText={`Submit to ${supplierName}`} />
            </CardContent>
            </form>
        </Card>
